...
from model_Set import SE_block,SK_block
...
def shuffle_block_s1(inputs, output_c: int, stride: int, prefix: str):
    ...
    x1, x2 = ChannelSplit(name=prefix + "/split")(inputs) # 通道切分处理，x1捷径分支，x2待处理分支

    # main branch
    x2 = ConvBNReLU(filters=branch_c, name=prefix + "/b2_conv1")(x2) #1*1卷积
    # x2 = DWConvBN(kernel_size=3, strides=stride, name=prefix + "/b2_dw1")(x2) #dw卷积
    x2 = SK_block(x2)
    x2 = layers.BatchNormalization()(x2)
    x2 = ConvBNReLU(filters=branch_c, name=prefix + "/b2_conv2")(x2) #1*1卷积

    x = layers.Concatenate(name=prefix + "/concat")([x1, x2]) #主、副分支拼接
...
def shufflenet_v2(num_classes: int, # 分类的类别个数
    ...
    if len(stages_repeats) != 3: # 网络具有三个阶段，不是三个阶段报错体系
...
# 搭建网络
def shufflenet_v2_x1_0(num_classes=1000, input_shape=(224, 224, 1)):
